...
def make_dataset(image_list, label_list, flag=0, au_relation=None):
    ...
    if flag>0.5:
        len_=(len_2)/2  #this line here because in dataset_process it loaded twice the paths for rafau, so len(image_list)=2*len(label_list)
...
class RAFAU(Dataset):
    def __init__(self, root_path, phase='train', transform=None, stage=1, loader=default_loader):
        ...
        if self._phase == 'train':
            # img
            train_image_list_path = os.path.join(root_path, 'list', 'RAF_AU_train_img_path.txt')
            train_image_list = open(train_image_list_path).readlines()
            # img labels
            train_label_list_path = os.path.join(root_path, 'list', 'RAF_AU_train_label.txt')
            train_label_list = np.loadtxt(train_label_list_path)

            # AU relation   -> still not implemented for AffWild2
            if self._stage == 2:
                au_relation_list_path = os.path.join(root_path, 'list', 'RAF_AU_train_AU_relation.txt')
                au_relation_list = np.loadtxt(au_relation_list_path)
                self.data_list = make_dataset(train_image_list, train_label_list, au_relation_list)
            else:
                self.data_list = make_dataset(train_image_list, train_label_list)
            ...
            # img
            eval_image_list_path = os.path.join(root_path, 'list', 'RAF_AU_val_img_path.txt')
            eval_image_list = open(eval_image_list_path).readlines()

            # img labels
            eval_label_list_path = os.path.join(root_path, 'list', 'RAF_AU_val_label.txt')
            eval_label_list = np.loadtxt(eval_label_list_path)
            self.data_list = make_dataset(eval_image_list, eval_label_list)

        else:
            # img
            eval_image_list_path = os.path.join(root_path, 'list', 'RAF_AU_test_img_path.txt')
            eval_image_list = open(eval_image_list_path).readlines()
...
class CASME2(Dataset):
    def __init__(self, root_path, phase='train', transform=None, stage=1, loader=default_loader):
        ...
        if self._phase == 'train':
            # img
            train_image_list_path = os.path.join(root_path, 'list', 'CASME2_train_img_path.txt')
            train_image_list = open(train_image_list_path).readlines()
            # img labels
            train_label_list_path = os.path.join(root_path, 'list', 'CASME2_train_label.txt')
            train_label_list = np.loadtxt(train_label_list_path)

            # AU relation   -> still not implemented for AffWild2
            if self._stage == 2:
                au_relation_list_path = os.path.join(root_path, 'list', 'CASME2_train_AU_relation.txt')
                au_relation_list = np.loadtxt(au_relation_list_path)
                self.data_list = make_dataset(train_image_list, train_label_list, au_relation_list)
            else:
                self.data_list = make_dataset(train_image_list, train_label_list)
            ...
            # img
            eval_image_list_path = os.path.join(root_path, 'list', 'CASME2_val_img_path.txt')
            eval_image_list = open(eval_image_list_path).readlines()

            # img labels
            eval_label_list_path = os.path.join(root_path, 'list', 'CASME2_val_label.txt')
            eval_label_list = np.loadtxt(eval_label_list_path)
            self.data_list = make_dataset(eval_image_list, eval_label_list)

        else:
            # img
            eval_image_list_path = os.path.join(root_path, 'list', 'CASME2_test_img_path.txt')
            eval_image_list = open(eval_image_list_path).readlines()

            # img labels
            eval_label_list_path = os.path.join(root_path, 'list', 'CASME2_test_label.txt')
            eval_label_list = np.loadtxt(eval_label_list_path)  #  -> to change when we understand how to do it automatically
            self.data_list = make_dataset(eval_image_list, eval_label_list)
