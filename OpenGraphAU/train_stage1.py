...
def get_dataloader(conf):
    ...
    trainset = AffWild2(conf.dataset_path, phase='train', transform=image_train(crop_size=conf.crop_size), stage = 1)
    train_loader = DataLoader(trainset, batch_size=conf.batch_size, shuffle=True, num_workers=conf.num_workers)
    valset = AffWild2(conf.dataset_path, phase='val', transform=image_eval(crop_size=conf.crop_size), stage = 1)
